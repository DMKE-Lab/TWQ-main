...
model = {
    ...
    'TWQ_main':TWQ_main(sizes, args.rank, cycle=args.cycle, is_cuda=True if args.gpu == 1 else False),
}[args.model]
...
for epoch in range(args.max_epochs):
    ...
    def avg_both(mr, mrrs: Dict[str, float], hits: Dict[str, torch.FloatTensor], added):
        ...
        :return:
        """
        #m = (mrrs['lhs'] + mrrs['rhs']) / 2.  #1p
        m = mrrs['rhs']#2p
        #h = (hits['lhs'] + hits['rhs']) / 2.  #1p
        h = hits['rhs']
        #mr = (mr['lhs'] + mr['rhs']) / 2.  #1p
        mr = mr['rhs']
    ...
    if epoch < 0 or (epoch + 1) % args.valid_freq == 0:
        if dataset.has_intervals():
            valid, test = [
                ...
                for split in ['valid-easy', 'test-easy']
                #for split in ['valid-hard', 'test-hard']
            ]
            print("valid: ", valid)
            print("test: ", test)
        else:
