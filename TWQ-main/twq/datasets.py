...
class TemporalDataset(object):
    def __init__(self, name: str, is_cuda: bool = False):
        ...
        maxis2 = np.max(self.data['test'], axis=0)#easy OR hard
        ...
        if maxis.shape[0] > 4:
            self.n_timestamps = max(int(maxis[3] + 1), int(maxis[4] + 1)) #1p
